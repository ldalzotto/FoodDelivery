...
class UserService
{
    ...
    {
        Server.SendRequest_Json("POST", "http://localhost:8080/user/register", p_user, false, 
            (pp_user : User) => {
    ...
    {
        Server.SendRequest_Json("GET", "http://localhost:8080/user", null, true, p_ok, p_err);
    }

    static Validate(p_userId : string, p_sessionToken : string, p_ok ?: ()=>void, p_err ?: (err:ServerError)=>void)
    {
        Server.SendRequest_Json("POST", `http://localhost:8080/user/validate?userId=${p_userId}&sessionToken=${p_sessionToken}`, null, false,
            p_ok, p_err);
    }
}
