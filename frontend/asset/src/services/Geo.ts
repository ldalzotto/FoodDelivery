...
class GeoService
{
    ...
    {
        Server.SendRequest_Json("GET", `http://localhost:8080/city/match?matchValue=${p_matchingName}&limit=${p_limit}`, null, false
    ...
    {
        Server.SendRequest_Json("GET", `http://localhost:8080/city?city_id=${p_city_id}`, null, false, 
            (p_body:City) => {p_onCompleted(p_body, null);}, (p_err : ServerError)=>{p_onCompleted(null, p_err);});
    }
